...
function getNames(people) {
  ...
  for (let i = 0; i < people.length; i++) {
    if (people[i].firstName && people[i].lastName) {
...
//console.log( Karl Cliffgate, Avernus)
...
function translateToPirateTalk(phrase) {
  ...
  for (let i = 0; i < phraseArr.length; i++) {
    let word = phraseArr[i];
    if (EN_PIRATE_LOOKUP[word]) {
      piratePhrase.push(EN_PIRATE_LOOKUP[word]);
    } else {
...
function wordCount(str) {
  const words = str.split(" "); //split into a list of words so that you can iterate over the words.
  let number = {};
  words.forEach((word) => {
    number[word] = number[word] ? number[word] + 1 : 1; //ternary
  });
  return number;
}
...
function isBugAvailable(bug, month) {
  const monthsArray = bug.availability.months;
